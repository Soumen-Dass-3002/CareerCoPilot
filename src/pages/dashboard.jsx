...
function Dashboard() {
  ...
  const [showAuth, setShowAuth] = useState(() => localStorage.getItem("cc-auth-popup-dismissed") !== "true");
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      ...
      <nav className="side-nav" aria-label="Main navigation"><a className="active" href="#chat"><span>◈</span> My co-pilot</a><Link to="/simulator"><span>⌁</span> Explore your path</Link><Link to="/careers"><span>◌</span> Explore careers</Link><Link to="/colleges"><span>⌂</span> Explore colleges</Link><Link to="/resumes"><span>▤</span> Resume library</Link><Link to="/jobs"><span>◉</span> Job finder</Link><Link to="/applications"><span>✓</span> Applications</Link></nav>
      <div className="journey-card"><div className="tiny-label">YOUR JOURNEY</div><div className="journey-row"><span>Explorer</span><strong>Level 1</strong></div><div className="progress"><span /></div><p>80 XP to level 2</p></div>
        <div className="profile-row"><div className="avatar">A</div><div><strong>Aanya Sharma</strong><small>Student</small></div><Link to="/profile" aria-label="Edit profile">···</Link></div>
    </aside>
    <main className="workspace">
      ...
      <div className="content-grid">
        ...
        <aside className="right-rail"><section className={`quest-card ${gameStarted ? "started" : ""}`} id="path"><span className="game-kicker">LEARN BY PLAYING</span><div className="quest-art">🕹️ <span>✨</span></div><h2>{gameStarted ? "Your first quest awaits!" : "Not sure what to ask?"}</h2><p>{gameStarted ? "Answer quick choices, earn XP, and uncover career paths made for you." : "Play a short adventure to discover what makes you, you."}</p><button onClick={() => setGameStarted(true)}>{gameStarted ? "CONTINUE QUEST →" : "PLAY CAREER QUEST →"}</button><small>5 min · No wrong answers</small></section><section className="snapshot-card" id="discover"><div className="snapshot-title"><h3>Your snapshot</h3><Link to="/profile">View all →</Link></div><Link className="snapshot-item" to="/resumes"><span className="snapshot-icon purple">✧</span><div><strong>Resume & ATS</strong><p>Build or analyse your resume</p></div><span className="chevron">›</span></Link><Link className="snapshot-item" to="/jobs"><span className="snapshot-icon orange">⌁</span><div><strong>Recommended jobs</strong><p>Explore demo opportunities</p></div><span className="chevron">›</span></Link></section></aside>
      </div>
    </main>
    {showAuth && <div className="auth-overlay" role="dialog" aria-modal="true" aria-labelledby="auth-title">
      <div className="welcome-modal">
        <button className="modal-close" onClick={dismissAuth} aria-label="Continue exploring without an account">×</button>
        ...
        <button className="explore-button" onClick={dismissAuth}>I’LL EXPLORE FIRST</button>
      </div>
    </div>}
  </div>;
}
